 ...
 */
abstract class FrontifyMediaFormBase extends AddFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if (empty($added_media)) {
      ...
      $form['description'] = [
        ...
        '#template' => '<p>{{ text }}</p>',
        '#context' => [
          'text' => $this->formatPlural(count($added_media), 'The Frontify asset has been imported but not yet saved. Fill any extra field. Clicking on Insert saves it.', 'The Frontify assets have been imported but not yet saved. Fill any extra field. Clicking on Insert saves assets.'),
        ],
      ];
  ...
  protected function buildInputElement(array $form, FormStateInterface $form_state) {
    ...
    $mediaType = $this->getMediaType($form_state);
    $mediaTypeConfiguration = $mediaType->getSource()->getConfiguration();
    if (!empty($mediaTypeConfiguration['allowed_extensions'])) {
      $frontifyUiConfig->allowed_extensions = explode(' ', trim($mediaTypeConfiguration['allowed_extensions']));
    }
